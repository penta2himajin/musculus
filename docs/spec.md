...
| M4 | Irodori 比較。**完了**(2026-09-13):盲検 CMOS 5/5・平均 +2.75 → **二本立て採用**(ADR-0005)。製品経路(`factory`/CLI `--engine`/辞書両対応/CER 一般化)も完了。**5 steps で実時間超え(RTF 0.83)・辞書適用で CER 収束**を実測 — 低ステップの聴感検証が次の判断点 | 両エンジンが CLI から使える ✓ |
...
- **SBV2 レバーの文書化と実装(2026-09-15)**:[docs/sbv2-levers.md](sbv2-levers.md) — 上流の既定値(sdp_ratio 0.2 / noise 0.6)と我々の既定(0.0 / 0.677 = Rust 参照)の差、**tsukuyomi は style 1 行のため style レバーは no-op**、`given_phone/given_tone` は上流のアクセント制御 API。CLI に `--sdp-ratio` / `--noise-scale` / `--noise-scale-w` / `--length-scale` を追加。実測: sdp 0.0 は長さが決定的、0.2 は変動(テンポの緩急)。聴取セット `listening/09-sdp-ratio/` で判定待ち
