...
- **アクセント逸脱層(2026-09-14 実装、既定 off/opt-in)**:`JaProcess::apply_accent_deviations`(NJD ノードのアクセント欄を書き換え)+ CLI/`eval_cer` の `--accent-deviations`、`accent_report --deviations`。**規則1=丁寧接頭辞 お/ご + 平板名詞**(参照は acc=2 を付けてチュ後に下降するが、辞書は注文=0/4、規範も接頭辞は結合相手のアクセントを変えない → ゴL+全て高に修正。ご注文・お名前・お勉強・ご連絡で確認)。数詞規則は「平板 HH/LL に届かない」ため削除(句レベル制御が次の課題)
